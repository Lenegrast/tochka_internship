...
def solve(lines: List[str]) -> int:
    ...
    def get_valid_moves(hallway: Tuple[str, ...], rooms: Tuple[Tuple[str, ...], ...]) -> List[Tuple]:
        ...
        for hall_pos in range(11):
            if hallway[hall_pos] != '.':
                ...
                if can_enter_room(rooms, target_room, obj_type) and is_path_clear(hallway, hall_pos, room_hall_pos):
        ...
        for room_idx in range(4):
            ...
            if TARGET_ROOMS[obj_type] == room_idx and all(room[d] == obj_type for d in range(top_depth, room_depth)):
                continue
    ...
    def apply_move(hallway: Tuple[str, ...], rooms: Tuple[Tuple[str, ...], ...], move: Tuple) -> Tuple[Tuple[str, ...], Tuple[Tuple[str, ...], ...], int]:
        move_type, *args = move
