...
def parse_input(lines):
    ...
    for d in range(room_depth):
        room_line = lines[3 + d][1:-1].split('#')
        for r in range(4):
            if room_line[r] != '.':
                rooms[r][d] = room_line[r]
...
def is_final_state(state, room_depth):
    ...
    for r in range(4):
        room_start = 11 + r * room_depth
        for d in range(room_depth):
            obj = state[room_start + d]
...
def can_move_to_room(state, obj, room, room_depth):
    ...
    if ROOM_FOR_TYPE[obj] != room:
        return None
    room_start = 11 + room * room_depth

    for d in range(room_depth):
        if state[room_start + d] not in (None, obj):
            return None

    for d in range(room_depth - 1, -1, -1):
        if state[room_start + d] is None:
            return d
    return None
...
def get_moves(state, room_depth):
    ...
    for room in range(4):
        ...
        for depth in range(room_depth):
            obj = state[room_start + depth]
            if obj is not None:
                for target_pos in HALLWAY_STOPS:
                    if path_clear(state, ROOM_POS[room], target_pos):
                        
                        steps = (depth + 1) + abs(ROOM_POS[room] - target_pos)
                        cost = steps * COSTS[obj]
                        
                        new_state = list(state)
                        new_state[room_start + depth] = None
                        new_state[target_pos] = obj
                        moves.append((tuple(new_state), cost))
                break
